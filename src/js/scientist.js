...
buttons.forEach(button => {
	button.addEventListener('click', () => {
		...
		switch (action) {
			case 'born-19':
				filteredScientists = filteredScientists.filter(
			...
			case 'sort-alpha':
				...
				break
			case 'surname-c':
				filteredScientists = filteredScientists.filter(scientist =>
			...
			case 'youngest':
				filteredScientists = [
					filteredScientists.reduce((youngest, scientist) =>
						scientist.born > youngest.born ? scientist : youngest
					)
				]
				break
			case 'longest-shortest':
				const longest = filteredScientists.reduce((a, b) =>
					a.dead - a.born > b.dead - b.born ? a : b
				)
				const shortest = filteredScientists.reduce((a, b) =>
					a.dead - a.born < b.dead - b.born ? a : b
				)
